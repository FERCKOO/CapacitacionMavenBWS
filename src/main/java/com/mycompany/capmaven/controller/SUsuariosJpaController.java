 ...
 * @author Blueweb
 */
public class SUsuariosJpaController extends HttpServlet {
    ...
    // Trae usuario y contraseña para verificarlo en la BD
    public Boolean VerificarConexion(SUsuarios user) {
        EntityManager em = getEntityManager();
        Boolean rs = false;

        try {
            Query query = em.createNamedQuery("SUsuarios.findUserPassword", SUsuarios.class);
            query.setParameter("usuario", user.getUsuario());
            query.setParameter("password", HexDigest.hexDigest(user.getPassword()));

            List<SUsuarios> lista = query.getResultList();

            if (lista.toArray().length > 0) {
                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("usuario", lista.get(0));

                user = (SUsuarios) FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("usuario");
                user.setIdUsuarioModifica(user);
